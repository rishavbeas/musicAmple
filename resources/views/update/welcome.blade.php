        ...
        <div class="h-100 flex flex-col justify-center items-center my-6">
            <div class="relative w-32 h-32 flex items-center justify-center">
                ...
                <div class="absolute right-0 bottom-0 bg-green-500 w-8 h-8 rounded-full flex items-center justify-center">
                    @include('icons.ellipsis-horizontal', ['class' => 'font-light text-green-100 w-4 h-4'])
                </div>
            </div>

            <div>
                <h5 class="mt-4 text-center font-medium dark:text-white">{{ __('Update') }}</h5>
                <p class="text-center mb-0 dark:text-white">{!! __(':name update wizard.', ['name' => '<span class="font-medium">'.config('info.software.name').'</span>']) !!}</p>
            </div>
        </div>
    </div>
    <a href="{{ route('update.overview') }}">
        <button type="button"
