@section('site_title', formatTitle([__('Tracks'), config('app.name')]))
<x-app-layout>
    <x-slot name="header">
        <div class="flex mb-4">
            <div class="w-1/2">
            ...
            <div class="bg-white dark:bg-gray-800 shadow-sm sm:rounded-lg p-4">
                <div class="sm:flex sm:items-center sm:justify-between py-4 bg-white dark:bg-gray-800">
                    ...
                    <div cclass="flex items-center mt-4 gap-x-3">
                        ...
                        <form method="GET" action="{{ route('tracks') }}">
                            <div class="relative">
                                ...
                                <input type="text" name="search" value="{{ app('request')->input('search') }}"
                                    ...
                                    placeholder="{{ __('Search for tracks') }}">
                                                ...
                                                <div class="px-4 py-2">
                                                    ...
                                                    <div class="relative mt-2 rounded-md shadow-sm">
                                                        <select name="status" id="i-status"
                                                            ...
                                                            @foreach ([1 => __('Public'), 2 => __('Unpublic')] as $key => $value)
                                                                <option value="{{ $key }}"
                                                ...
                                                <div class="px-4 py-2">
                                                    ...
                                                    <div class="relative mt-2 rounded-md shadow-sm">
                                                        <select name="sort_by" id="i-sort-by"
                                                            class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500">
                                                            @foreach (['id' => __('Date created'), 'name' => __('Name'), 'views' => __('View'), 'downloads' => __('Download')] as $key => $value)
                                                                <option value="{{ $key }}"
                            ...
                            <tbody>
                                @foreach ($tracks as $track)
                                    ...
                                    <tr
                                        ...
                                        <td class="px-6 py-4">
                                            {{ number_format_short($track->views) }}
                                        </td>
                                        <td class="px-6 py-4">
                                            {{ number_format_short($track->downloads) }}
                                        </td>
                                        <td class="px-6 py-4">
                                            {{ $track->created_at->tz(config('settings.timezone'))->format(__('Y-m-d')) }}
                                        </td>
                                        <td>
                                            <div class="flex items-center">
                                                <div
                                                    class="h-2.5 w-2.5 rounded-full {{ $track->public == 1 ? 'bg-green-500' : 'bg-red-500' }} mr-2">
                                                </div>
                                                @if ($track->public == 1)
                                                    {{ __('Publish') }}
                                                @else
                                        ...
                                        <td>
                                            <div class="sm:flex sm:items-center sm:ml-6">
                                                <x-dropdown align="right" width="48">
                                                    ...
                                                    <x-slot name="content">
                                                        <x-dropdown-link :href="route('tracks.edit', $track->id)" class="flex">
                                                        ...
                                                        <x-dropdown-link href="#" class="flex"
                                                            x-on:click.prevent="$dispatch('open-modal', { name: 'modal-action', url: '{{ route($track->public == 1 ? 'tracks.unpublic' : 'tracks.public', $track->id) }}' , title : '{{ $track->public == 2 ? __('Public') : __('Unpublic') }}', subTitle: '{{ __($track->public == 2 ? 'Are you sure you want to public :name ?' : 'Are you sure you want to unpublic :name ?', ['name' => $track->title]) }}' , action: '{{ $track->public == 2 ? __('Publish') : 'Unpublish' }}' })">
                                                            @if ($track->public == 2)
                                                                @include('icons.eye', [
                                                            ...
                                                            @endif
                                                            <span
                                                                class="mx-1 {{ $track->public == 2 ? 'text-green-500 ' : 'text-red-500 ' }}">
                                                                {{ $track->public == 2 ? 'Publish' : 'Unpublish' }}
                                                            </span>
                                                        </x-dropdown-link>
                                                        <x-dropdown-link href="#" class="flex"
                                                            x-on:click.prevent="$dispatch('open-modal', { name: 'modal-action', url: '{{ route('tracks.destroy', $track->id) }}' , title: '{{ __('Delete') }}' , subTitle : '{{ __('Are you sure you want to delete :name ?', ['name' => $track->title]) }}', action: '{{ __('Delete') }}' })">
