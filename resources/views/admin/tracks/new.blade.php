@section('site_title', formatTitle([__('Upload'), config('app.name')]))
<x-app-layout>
    <x-slot name="header">
                    ...
                    <section x-data="UploadForm()" class="space-y-6 text-gray-900 dark:text-gray-100">
                        ...
                        @php
                            ...
                            if (count($albums)) {
                                ...
                                $album = explode(',', $album_list[0]);
                                $selectedAlbum = $album[0] . ',' . $album[1]. ',' . $album[3];
                            }
                        @endphp
                        ...
                        <form @submit.prevent="submitForm" class="mt-6 space-y-6" enctype="multipart/form-data">
                            ...
                            <div class="flex items-center gap-4">
                                <x-primary-button x-bind:disabled="loading ? true : false"
                                    x-text="loading ? '{{ __('Processing') }}' : '{{ __('Upload') }}'">
                                </x-primary-button>
                            </div>
                        </form>
                    </section>
...
<script>
    const FORM_URL = '{{ route('tracks.new') }}';
    let artistId = [];
    let art = '';
    ...
    function UploadForm() {
        return {
            ...
            submitForm() {
                //Validate track
                if (this.track === "") {
                    this.message = "Please select the track"
                    this.uploadFailed = true
                    return
                }
                this.message = ""
                this.formData.artistId = artistId
                ...
                axios.post(FORM_URL, data, config).then((res) => {
                        if (res.status == 200) {
                            ...
                            this.loading = false
                            this.selectArtist = false
                            this.message = "Your track <strong>" + this.formData.title +
                                "</strong> has been uploaded"
                            //Reset Form
                            this.formData.title = ""
                            this.formData.description = ""
                            this.formData.artistId = []
                            this.lyric = ""
                            this.track = ""
                            this.image = ""
                            this.selected = {}
