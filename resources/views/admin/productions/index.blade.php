@section('site_title', formatTitle([__('Productions'), config('app.name')]))
<x-app-layout>
    <x-slot name="header">
        <div class="flex mb-4">
            <div class="w-1/2">
            ...
            <div class="bg-white dark:bg-gray-800 shadow-sm sm:rounded-lg p-4">
                <div class="sm:flex sm:items-center sm:justify-between py-4 bg-white dark:bg-gray-800">
                    ...
                    <div cclass="flex items-center mt-4 gap-x-3">
                        ...
                        <form method="GET" action="{{ route('productions') }}">
                            <div class="relative">
                                ...
                                <input type="text" name="search" value="{{ app('request')->input('search') }}"
                                    ...
                                    placeholder="{{ __('Search for productions') }}">
                                    ...
                                    <tr
                                        ...
                                        <th scope="row"
                                            ...
                                            <img class="w-10 h-10 rounded-full object-cover"
                                                src="{{ url('/') }}/uploads/productions/{{ $production->image }}"
                                                alt="{{ $production->name }}">
                                            <div class="pl-3">
                                        ...
                                        <td class="px-6 py-4">
                                            <div class="flex items-center">
                                                <div
                                                    class="h-2.5 w-2.5 rounded-full {{ $production->public == 2 ? 'bg-red-500' : 'bg-green-500' }} mr-2">
                                                </div>
                                                @if ($production->public == 1)
                                                    {{ __('Publish') }}
                                                @else
                                                    {{ __('Unpublish') }}
                                                @endif
                                        ...
                                        <td class="px-6 py-4">
                                            <div class="sm:flex sm:items-center sm:ml-6">
                                                <x-dropdown align="right" width="48">
                                                    ...
                                                    <x-slot name="content">
                                                        <x-dropdown-link :href="route('productions.edit', $production->id)" class="flex">
                                                        ...
                                                        <x-dropdown-link href="#" class="flex"
                                                            x-on:click.prevent="$dispatch('open-modal', { name: 'modal-action', url: '{{ route($production->public == 1 ? 'productions.unpublic' : 'productions.public', $production->id) }}' , title : '{{ $production->public == 2 ? __('Public') : __('Unpublic') }}', subTitle: '{{ __($production->public == 2 ? 'Are you sure you want to public :name ?' : 'Are you sure you want to unpublic :name ?', ['name' => $production->name]) }}' , action: '{{ $production->public == 2 ? __('Publish') : 'unpublish' }}' })">
                                                            @if ($production->public == 0)
                                                                @include('icons.eye', [
                                                            ...
                                                            @endif
                                                            <span
                                                                class="mx-1 {{ $production->public == 0 ? 'text-green-500 ' : 'text-red-500 ' }}">
                                                                {{ $production->public == 0 ? 'Publish' : 'Unpublish' }}
                                                            </span>
                                                        </x-dropdown-link>
                                                        <x-dropdown-link href="#" class="flex"
                                                            x-on:click.prevent="$dispatch('open-modal', { name: 'modal-action', url: '{{ route('productions.destroy', $production->id) }}' , title: '{{ __('Delete') }}' , subTitle : '{{ __('Are you sure you want to delete :name ?', ['name' => $production->name]) }}', action: '{{ __('Delete') }}' })">
