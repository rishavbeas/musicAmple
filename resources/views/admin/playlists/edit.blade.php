@section('site_title', formatTitle([__('Edit'), __('Playlists'), config('app.name')]))
<x-app-layout>
    <x-slot name="header">
            ...
            <div class="p-4 sm:p-8 bg-white dark:bg-gray-800 shadow sm:rounded-lg">
                <div class="max-w-xl" x-data="playlist()">
                    <section class="space-y-6">
                        ...
                        <form enctype="multipart/form-data" method="post" action="{{ route('playlists.edit',$playlist->id) }}" class="mt-6 space-y-6">
                            @csrf
                                ...
                                <div class="grow space-y-6">
                                    <div>
                                        ...
                                        <x-text-input id="name" name="name" type="text"
                                            class="mt-1 block w-full" :value="old('name', $playlist->name)" required autofocus />
                                        <x-input-error class="mt-2" :messages="$errors->get('name')" />
                                    </div>
                                    <div>
                                        <x-input-label for="description" :value="__('Description')" />
                                        <x-text-area id="description" name="description" type="text"
                                            class="mt-1 block w-full" :value="old('description', $playlist->description)" autofocus />
                                        <x-input-error class="mt-2" :messages="$errors->get('description')" />
                                    </div>
                                    <div>
                                        <x-input-label for="i-status" :value="__('Status')" />
                                        <x-input-select id="i-status" name="public" class="mt-1 block w-full"
                                            :options="[2 => __('Private'), 1 => __('Public')]" :selected="old('public',$playlist->public)" autofocus />
...
<script>
    function playlist() {
        return {
            ...
            init() {
                @if ($playlist->image)
                this.imageUrl = "{{ url('/') }}/uploads/playlists/{{ $playlist->image }}",
            ...
            fireTagsUpdateEvent() {
                ...
                this.selectedItem = JSON.stringify(this.selectedLists)
            },
            remove(id) {
