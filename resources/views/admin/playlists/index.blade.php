@section('site_title', formatTitle([__('Playlists'), config('app.name')]))
<x-app-layout>
    <x-slot name="header">
        <div class="flex mb-4">
            <div class="w-1/2">
            ...
            <div class="bg-white dark:bg-gray-800 shadow-sm sm:rounded-lg p-4">
                <div class="sm:flex sm:items-center sm:justify-between py-4 bg-white dark:bg-gray-800">
                    ...
                    <div cclass="flex items-center mt-4 gap-x-3">
                        ...
                        <form method="GET" action="{{ route('playlists') }}">
                            <div class="relative">
                                ...
                                <input type="text" name="search" value="{{ app('request')->input('search') }}"
                                    ...
                                    placeholder="{{ __('Search for playlists') }}">
                                                ...
                                                <div class="px-4 py-2">
                                                    ...
                                                    <div class="relative mt-2 rounded-md shadow-sm">
                                                        <select name="search_by" id="i-search-by"
                                                            ...
                                                            @foreach (['name' => __('Name'), 'description' => __('Description')] as $key => $value)
                                                                <option value="{{ $key }}"
                                                ...
                                                <div class="px-4 py-2">
                                                    ...
                                                    <div class="relative mt-2 rounded-md shadow-sm">
                                                        <select name="status" id="i-status"
                                                            ...
                                                            @foreach ([1 => __('Public'), 2 => __('Private')] as $key => $value)
                                                                <option value="{{ $key }}"
                                    ...
                                    <tr
                                        ...
                                        <th scope="row"
                                            ...
                                            <a href="{{ route('playlists.edit', $playlist->id) }}"
                                                class="flex"><img class="w-10 h-10 rounded-lg object-cover"
                                                    src="{{ $playlist->image ? URL::to('/uploads/playlists/' . $playlist->image) : URL::to('/uploads/playlists/default.png') }}"
                                                    alt="{{ $playlist->title }}">
                                                <div class="pl-3 max-w-sm">
                                                    <div class="text-base font-semibold text-ellipsis overflow-hidden">
                                                        {{ $playlist->name }}
                                                    </div>
                                        ...
                                        <td class="px-6 py-4">
                                            {{ $playlist->created_at->tz(config('settings.timezone'))->format(__('Y-m-d')) }}
                                        </td>
                                        <td class="px-6 py-4">
                                            <div class="flex items-center">
                                                <div
                                                    class="h-2.5 w-2.5 rounded-full {{ $playlist->public == 2 ? 'bg-red-500' : 'bg-green-500' }} mr-2">
                                                </div>
                                                {{ $playlist->public == 1 ? 'Public' : 'Private' }}
                                            </div>
                                        </td>
                                        <td class="px-6 py-4">
                                            <div class="sm:flex sm:items-center sm:ml-6">
                                                <x-dropdown align="right" width="48">
                                                    ...
                                                    <x-slot name="content">
                                                        <x-dropdown-link :href="route('playlists.edit', $playlist->id)" class="flex">
                                                        ...
                                                        <x-dropdown-link href="#" class="flex"
                                                            x-on:click.prevent="$dispatch('open-modal', { name: 'modal-action', url: '{{ route($playlist->public == 1 ? 'playlists.private' : 'playlists.public', $playlist->id) }}' , title : '{{ $playlist->public == 1 ? __('Public') : __('Private') }}', subTitle: '{{ __($playlist->public == 2 ? 'Are you sure you want to public :name ?' : 'Are you sure you want to private :name ?', ['name' => $playlist->name]) }}' , action: '{{ $playlist->public == 2 ? __('Publish') : 'Private' }}' })">
                                                            ...
                                                            @endif
                                                            <span
                                                                class="mx-1 {{ $playlist->public == 2 ? 'text-green-500 ' : 'text-red-500 ' }}">
                                                                {{ $playlist->public == 2 ? 'Publish' : 'Private' }}
                                                            </span>
                                                        </x-dropdown-link>
                                                        <x-dropdown-link href="#" class="flex"
                                                            x-on:click.prevent="$dispatch('open-modal', { name: 'modal-action', url: '{{ route('playlists.destroy', $playlist->id) }}' , title: '{{ __('Delete') }}' , subTitle : '{{ __('Are you sure you want to delete :name ?', ['name' => $playlist->name]) }}', action: '{{ __('Delete') }}' })">
