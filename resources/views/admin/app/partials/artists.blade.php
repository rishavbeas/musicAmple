<div>
    <x-input-label :value="__('Select Artist')" />
    <div x-data @tags-update="item= $event.detail.item" data-tags="[]">
        <input type="hidden" x-model="selectedItem" name="artists"/>
        <div class="relative">
            <div class="border border-gray-300 dark:border-gray-700 dark:bg-gray-900 rounded-md p-2 flex gap-1 flex-wrap mt-2"
                ...
                <div class="flex-1">
                    <input type="text" x-model="search" x-ref="search_input"
                        @input.debounce.400ms="goSearch(1);"
                        placeholder="{{ __('Search') }}"
                        x-bind:required="isSelectItem ? false : true"
