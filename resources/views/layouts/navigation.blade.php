<nav x-data="{ open: false }" class="bg-white dark:bg-gray-800 border-b border-gray-100 dark:border-gray-700">
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="flex justify-between h-16">
            <div class="flex">
                ...
                <div class="hidden space-x-8 sm:-my-px sm:ml-8 sm:flex">
                    <x-nav-link :href="route('productions')" :active="request()->routeIs('productions')">
                        {{ __('Productions') }}
                    </x-nav-link>
                </div>
                <div class="hidden space-x-8 sm:-my-px sm:ml-8 sm:flex">
                ...
                <div class="hidden sm:flex sm:items-center sm:ml-8">
                    <x-dropdown align="right" width="48">
                        ...
                        <x-slot name="content">
                            <x-dropdown-link :href="route('pages')" :active="request()->routeIs('pages')">
                                {{ __('Pages') }}
                            </x-dropdown-link>
                            <x-dropdown-link :href="route('reports')" :active="request()->routeIs('reports')">
                                {{ __('Reports') }}
                            </x-dropdown-link>
                            <x-dropdown-link :href="route('app')" :active="request()->routeIs('app')">
                                {{ __('App') }}
                            </x-dropdown-link>
            ...
            <div class="hidden sm:flex sm:items-center sm:ml-6">
                <x-dropdown align="right" width="48">
                    <x-slot name="trigger">
                        <button class="inline-flex items-center px-3 py-2 border border-transparent text-sm leading-4 font-medium rounded-md text-gray-500 dark:text-gray-400 bg-white dark:bg-gray-800 hover:text-gray-700 dark:hover:text-gray-300 focus:outline-none transition ease-in-out duration-150">
                            <img class="w-6 h-6 mr-2 rounded-full object-cover"
                                 src="{{ url('/') }}/uploads/avatars/{{ Auth::guard('admin')->user()->image }}"
                                 alt="{{ Auth::guard('admin')->user()->username  }}">
                            <div>{{ Auth::guard('admin')->user()->username
                                }}
                            </div>
                    ...
                    <x-slot name="content">
                        ...
                        <!-- Authentication -->
                        <form method="POST" action="{{ route('admin.custom.logout') }}">
                            @csrf

                            <x-dropdown-link :href="route('admin.custom.logout')"
    ...
    <div :class="{'block': open, 'hidden': ! open}" class="hidden sm:hidden">
        <div class="pt-2 pb-3 space-y-1">
            ...
            <x-responsive-nav-link :href="route('productions')" :active="request()->routeIs('productions')">
                {{ __('Productions') }}
            </x-responsive-nav-link>
        ...
        <div class="pt-4 pb-1 border-t border-gray-200 dark:border-gray-600">
            ...
            <div class="mt-3 space-y-1">
                ...
                <form method="POST" action="{{ route('admin.custom.logout') }}">
                    ...
                    <x-responsive-nav-link :href="route('admin.custom.logout')"
                                           onclick="event.preventDefault();
                                        this.closest('form').submit();">
                        {{ __('Log Out') }}
                    </x-responsive-nav-link>
