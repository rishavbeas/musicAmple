    ...
    <div class="p-5">
        <div class="h-100 flex flex-col justify-center items-center my-6">
            <div class="relative w-32 h-32 flex items-center justify-center">
                ...
                @include('icons.arrow-down-on-square', ['class' => 'w-16 h-16 text-green-600 dark:text-green-400'])
            ...
            <div>
                <h5 class="mt-4 text-center font-medium dark:text-white">{{ __('Installed') }}</h5>
                <p class="text-center mb-0 dark:text-white">{!! __(':name has been installed.', ['name' => '<span class="font-medium">'.config('info.software.name').'</span>']) !!}</p>
            </div>
        </div>
    </div>
    <a href="{{ route('home') }}">
